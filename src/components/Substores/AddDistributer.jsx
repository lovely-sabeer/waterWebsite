...
function AddDistributer({ func }) {
    ...
    const handlePost = () => {
        fetch(url + '/distributer/addDistributers', {
                ...
                body: JSON.stringify({distributer, invoiceNo, date, ml500Qty, ml500Price, ml1000Qty, ml1000Price, ml1500Qty,  ml1500Price, ml5000Qty, ml5000Price, ml19000Qty, ml19000Price, paid, totalPrice, balance}) 
        })
        .then((res, err) => {
    ...
    return (
		<div className=' absolute w-4/6  border-2 border-black top-5 rounded-md p-7 bg-white z-30 flex flex-col items-center justify-center ' style={{ boxShadow: "5px 5px 20px black, -5px -5px 20px black" }}>
			...
			<div className=" py-4">
                <div className='text-xl font-bold underline'> Sold to Whole Seller </div>
			</div>
            <div className="flex flex-col w-full">
                <div className='flex items-center gap-6'>
					<label className='font-bold text-lg'>Bulk Buyer: </label>
					<input type="name" placeholder="DIstributer name" className='w-1/3 p-1 border-b-2' onChange={(e)=>setDistributer(e.target.value)} />
                </div>
